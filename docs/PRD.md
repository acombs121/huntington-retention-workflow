...
* **The Agent at Work:** Powered by the **Gemini Enterprise Agent Platform (fka Vertex AI Platform)** running `gemini-3.7-flash`, Book Scout natively processes the raw scanned documents. Utilizing multimodal visual layout understanding, it extracts:
  ...
  - Generates **visual bounding-box citations** locating the exact evidentiary clause on the page. The Deal Analysis workspace draws each region at its reported coordinates and quotes the extracted language; it renders what the resolver returned rather than reproducing the page image. **Elapsed agent time: seconds.**
...
| **03:30–06:00** | **The Agentic Proof (Multimodal Grounding & Dynamic Slider):** Click Marcus Vance. Watch Gemini 3.7 Flash extract LLC ownership with **visual bounding-box highlights**. Show missing sale price resolved via trailing NOI grounded by Vertex AI Search ($8.5M @ 7.5% cap rate). Adjust the indicative valuation slider live from $8.5M to $9.0M. | **Pane 2 & 3:** Document Grounding panel — selecting a beneficial owner moves the highlighted region on the page and swaps the quoted clause and its coordinates (`HBAN-CRE-VAULT-8821`, page 11 of 14). Live streaming token feedback; interactive slider dynamically re-computing net proceeds to $3.35M. | Gemini Enterprise Agent Platform (`gemini-3.7-flash`) native multimodal document understanding; Vertex AI Search grounding; Cloud Run `CalculatorTool`. | CTO & CIO see state-of-the-art multimodal AI; CFO sees automated analyst judgment. |
